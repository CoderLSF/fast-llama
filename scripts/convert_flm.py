...
class TensorLoader:
    ...
    def quantize(arr:np.ndarray, data_type: DataType, group_size=64):
        ...
        scales = arr.max(axis=1) / scale_range
        arr /= scales[:, None]
        ...
        if data_type.is_int8:
            #print(f'arr.data_type:{arr.dtype} to np.int8')
    ...
    def _dump_tensors(self, outf: FLFWriter, out_type:str):
        ...
        for name, tensor_loader in self.tensor_map.items():
            ...
            if name == 'model.embed_tokens.weight':
                ...
                if layer_id == 0 and tensor_type == TensorType.LAYER_ATTN_Q:
                    shape = list(tensor.shape)
                    shape.reverse()
                    print(f'attn_q shape:{tuple(shape)}=> ')
                    print_ndarray(tensor)
                    #print_ndarray(tensor, f"{name}.txt")
            else:
